...
import type { EstadoSolicitudRepuesto, SolicitudAdmin } from '@/types/admin';

const COLUMN_ORDER: EstadoSolicitudRepuesto[] = [
    'nueva',
    'verificando_stock',
    'cotizada',
    'aprobada',
...
> = {
    nueva: {
        title: 'Nueva',
        description: 'Solicitud ingresada, sin procesar.',
    ...
    verificando_stock: {
        ...
        description: 'Consultando disponibilidad.',
    ...
    cotizada: {
        title: 'Cotizada',
        description: 'Precio enviado al cliente.',
    ...
    aprobada: {
        title: 'Aprobada',
        description: 'Cliente confirmo la compra.',
    ...
    en_preparacion: {
        ...
        description: 'Armando el pedido para entrega.',
    ...
    entregada: {
        title: 'Entregada',
        description: 'Repuesto retirado o despachado.',
...
type KanbanState = Record<EstadoSolicitudRepuesto, SolicitudAdmin[]>;

type DragData = {
    solicitudId: string;
    fromColumn: EstadoSolicitudRepuesto;
};
...
function isEstadoSolicitudRepuesto(value: string): value is EstadoSolicitudRepuesto {
    return COLUMN_ORDER.includes(value as EstadoSolicitudRepuesto);
}

function normalizeSolicitudes(solicitudes: SolicitudAdmin[]): KanbanState {
    return solicitudes.reduce<KanbanState>((acc, solicitud) => {
        const estado = isEstadoSolicitudRepuesto(solicitud.estado)
            ? solicitud.estado
            : 'nueva';
        acc[estado].push({ ...solicitud, estado });
        return acc;
    }, createEmptyBoard());
}
...
function getDragData(data: Record<string, unknown>): DragData | null {
    ...
    if (
        ...
        isEstadoSolicitudRepuesto(fromColumn)
    ) {
...
function truncateText(value: string | undefined, limit: number): string {
    if (!value) return 'Sin descripcion cargada.';
...
function RepuestoCard({
    ...
    solicitud: SolicitudAdmin & { estado: EstadoSolicitudRepuesto };
}) {
    ...
    const equipo = [solicitud.equipo_modelo, solicitud.equipo_serie].filter(Boolean).join(' / ');
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-xs text-zinc-500">
                ...
                {solicitud.cliente_id ? (
                    <Link
                        ...
                        className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 font-medium text-amber-700 transition hover:bg-amber-100"
                    >
                        Ver cliente
                    </Link>
                ) : null}
...
function Column({
    ...
    column: EstadoSolicitudRepuesto;
    solicitudes: (SolicitudAdmin & { estado: EstadoSolicitudRepuesto })[];
    onMove: (args: {
        solicitudId: string;
        fromColumn: EstadoSolicitudRepuesto;
        toColumn: EstadoSolicitudRepuesto;
    ...
    useEffect(() => {
        ...
        return dropTargetForElements({
            ...
            onDrop: ({ source }) => {
                ...
                if (!dragData || dragData.fromColumn === column) return;
                onMove({ solicitudId: dragData.solicitudId, fromColumn: dragData.fromColumn, toColumn: column });
            },
        });
    }, [column, onMove]);
    ...
    return (
        <section
            ...
            className={cn(
                ...
                isOver && 'border-amber-400 ring-2 ring-amber-200'
            )}
        >
            <div className="flex items-start justify-between gap-3">
                <div>
            ...
            <div className="mt-5 space-y-3">
                {solicitudes.length > 0 ? (
                    solicitudes.map(solicitud => (
                        <RepuestoCard key={solicitud.id} solicitud={solicitud} />
                    ))
                ) : (
...
export default function AdminRepuestosPage() {
    ...
    const loadSolicitudes = async (showLoader: boolean) => {
        ...
        try {
            ...
            const response = await fetch('/api/admin/solicitudes?tipo=repuesto', {
            ...
            setError(
                loadError instanceof Error ? loadError.message : 'No se pudieron cargar las solicitudes.'
            );
        } finally {
            setLoading(false);
            setRefreshing(false);
    ...
    const filteredBoard = useMemo<KanbanState>(() => {
        ...
        return COLUMN_ORDER.reduce<KanbanState>((acc, column) => {
            acc[column] = board[column].filter(solicitud => {
                ...
                return cliente.includes(term) || serie.includes(term) || desc.includes(term);
            });
            return acc;
        }, createEmptyBoard());
    }, [board, search]);

    const totalRepuestos = COLUMN_ORDER.reduce((total, column) => total + board[column].length, 0);
            ...
            <header className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-600">
                                Gestion de solicitudes de repuestos CASE IH, desde la consulta inicial
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Buscar por cliente, serie o repuesto"
                                className="h-11 w-full rounded-2xl border border-zinc-300 bg-white pl-9 pr-4 text-sm text-zinc-900 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-100 sm:w-72"
                            />
                        </div>
                        <button
                            type="button"
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3 text-sm">
                    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-2 text-zinc-700">
                        Total en tablero:{' '}
                        <span className="font-semibold text-zinc-950">{totalRepuestos}</span>
                    </div>
                    {search.trim() ? (
                        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-2 text-zinc-700">
                            Filtro activo:{' '}
                            <span className="font-semibold text-zinc-950">{search.trim()}</span>
                        </div>
                    ) : null}
                </div>
            </header>
